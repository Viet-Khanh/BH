...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    rows.forEach((item, index) => {
      ...
      const hasInvalidNumber = Number.isNaN(avgCost) || Number.isNaN(sellPriceDefault);
      const changes = {};
      ...
      if (!Object.keys(changes).length) {
        invalidRows.push({ rowNumber, name, reason: 'Thiếu giá vốn hoặc đơn giá lẻ.' });
        return;
      }
    ...
    const updatedProducts = await Promise.all(
      updates.map(({ product, changes }) =>
        Product.findOneAndUpdate({ id: product.id }, { $set: changes }, { new: true }).lean()
      )
    );
